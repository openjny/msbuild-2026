...
- [Work IQ](references/announcements/work-iq.md): エージェントが Microsoft 365 のデータ・アプリにアクセスするための新しいインテリジェンスレイヤー Work IQ と、その API セットを発表。Work IQ APIs は 2026年6月16日に GA 予定で、Chat・Context・Tools・Workspaces の4ドメインを提供。併せて Web IQ（エージェント向けウェブグラウンディング API スイート）も発表。
...
- [Windows Development](references/announcements/windows-dev.md): Windows の Build 2026 更新として、Windows プラットフォームセキュリティ（エージェント向け）、ローカル AI 機能、Surface RTX Spark Dev Box（NVIDIA RTX Spark 搭載、1 PFLOPS AI 計算能力）、Intelligent Terminal v0.1 を発表。開発者とエージェントのための信頼されたプラットフォームとしての Windows を強化。
