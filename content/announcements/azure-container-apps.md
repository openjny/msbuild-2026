...
- **Azure Container Apps Sandboxes**: エージェンティックワークロード向けセキュア分離インフラ
- **Container Apps Build 2026 更新**: プラットフォーム全体の機能強化

## 詳細

### Container Apps Sandboxes

AI エージェントが生成したコードの実行やウェブブラウジングを、メインのアプリケーション環境から分離して安全に実行する。Azure Functions のサーバーレスエージェントランタイムや GitHub Copilot のクラウドサンドボックスとも連携する基盤技術。

## 応用シナリオ

- AI エージェントが生成したコードのサンドボックス内実行と検証
- エージェンティックワークフローでのウェブブラウジング分離
- マルチテナント環境でのエージェント実行の安全なホスティング

## 参考リンク

- [What's new in Azure Container Apps at Build'26](https://techcommunity.microsoft.com/blog/appsonazureblog/whats-new-in-azure-container-apps-at-build26/4524184)
